...
class TackerAgent:
    ...
    def polling(self, vnf_id):
        """Constantly checks the status of a VNF.
    ...
    def vnf_init(self, vnf_ip, function):
        """Once Click VNF VM is fully created, initialize all tasks."""

        # send function to VNF VM
        self.em.write_function(vnf_ip, function)

        # start VNF function
        self.em.start_function(vnf_ip)
    ...
    def vnf_create(self, vnfd_data, vnf_name, click_function=None):
        ...
        if click_function:
            ...
            # TODO: What is the best way to determine this sleep time? It may change
            #       from one system to another.
            time.sleep(5)

            try:
                self.vnf_init(vnf_ip, click_function)
    ...
    def sfc_create(self, vnffgd: object, vnf_mapping: object) -> object:
        ...
        status, data = self.vnffg_create(vnffgd_id, vnf_mapping)
